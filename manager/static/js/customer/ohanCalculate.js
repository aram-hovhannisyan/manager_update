...
const ohanTot = document.getElementById('ohan')
const gavarTot = document.getElementById('gavar')
const avagTot = document.getElementById('avag')
const araTot = document.getElementById('ara')
ohanTot.textContent = 0
gavarTot.textContent = 0
avagTot.textContent = 0
araTot.textContent = 0
    ...
    rows.forEach(function(row) {
        ...
        inputs.forEach(function(input) {
            input.addEventListener("change", function() {
                ...
                if(input.getAttribute('customer')==='Օհան'){
                        ohanTot.textContent = parseInt(ohanTot.textContent) + total
                }
                else if(input.getAttribute('customer')==='Գ.4-րդ'){
                        gavarTot.textContent = parseInt(gavarTot.textContent) + total
                }       
                else if(input.getAttribute('customer')==='Գ.ավագ'){
                        avagTot.textContent = parseInt(avagTot.textContent) + total
                }         
                else if(input.getAttribute('customer')==='Արա'){
                        araTot.textContent = parseInt(araTot.textContent) + total
                }
